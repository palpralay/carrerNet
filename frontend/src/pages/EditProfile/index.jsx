...
const EditProfile = () => {
  ...
  useEffect(() => {
    ...
    dispatch(getAboutUser({ token })).then((res) => {
      if (res.payload) {
  ...
  const handleSaveUserInfo = async () => {
    ...
    try {
      ...
      if (result.type.includes('fulfilled')) {
        setMessage({ text: 'Basic info updated successfully!', type: 'success' });
        // Refresh user data
        dispatch(getAboutUser({ token: localStorage.getItem('token') }));
      } else {
        setMessage({ text: result.payload || 'Failed to update basic info', type: 'error' });
      }
    } catch (error) {
  ...
  const handleSaveProfileData = async () => {
    ...
    try {
      ...
      if (result.type.includes('fulfilled')) {
        ...
        setMessage({ text: result.payload || 'Failed to update profile', type: 'error' });
      }
    } catch (error) {
      setMessage({ text: 'An error occurred', type: 'error' });
    } finally {
  ...
  const handleUploadProfilePicture = async () => {
    ...
    try {
      ...
      if (result.type.includes('fulfilled')) {
        ...
        setMessage({ text: result.payload || 'Failed to upload picture', type: 'error' });
      }
    } catch (error) {
      setMessage({ text: 'An error occurred', type: 'error' });
    } finally {
